get_bsl_setting <- function(null_setting){
    x = copy(null_setting)
    x$parameter_values[["intercept_gender"]] <- 0.3
    x$parameter_values[["scale_PCP"]] <- 0.001
    x$parameter_values[["scale_death"]] <- 0.01
    x$parameter_values[["effect_trt_PCP"]] <- -0.4
    x$parameter_values[["effect_PCP_death"]] <- 0.4
    x$parameter_values[["effect_trt_death"]] <- -0.07
    x$parameter_values[["effect_gender_death"]] <- -0.7
    x$parameter_values[["effect_gender_crossover"]] <- -0.7
    x$parameter_values[["effect_gender_continue"]] <- 0.7
    x$parameter_values[["effect_continue_PCP"]] <- -1
    x$parameter_values[["effect_continue_death"]] <- -0.5
    x$parameter_values[["effect_karnofsky_death"]] <- -0.01
    x$parameter_values[["effect_karnofsky_PCP"]] <- -0.02
    x$parameter_values[["effect_continue_changekarnofsky"]] <- -0.7
